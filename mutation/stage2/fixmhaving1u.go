...
func doFixMHaving1U(rootNode ast.Node, in ast.Node) ([]byte, error) {
	...
	case *ast.SelectStmt:
		...
		if err != nil {
			return nil, errors.New("doFixMHaving1U: " +  err.Error())
		}
		// recover
		sel.Having.Expr = old
		return sql, nil
	case nil:
		return nil, errors.New("doFixMHaving1U: type error: nil")
	default:
		return nil, errors.New("doFixMHaving1U: type error: " + reflect.TypeOf(in).String())
	}
}